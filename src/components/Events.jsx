...
import { faAngleLeft, faAngleRight, faMapLocation, faCalendarDay } from "@fortawesome/free-solid-svg-icons";
...
const Events = () => {
	...
	useEffect(() => {
		const getTicket = async () => {
			try {
				const response = await axios.get("https://64bb45695e0670a501d6e6b5.mockapi.io/ticket");
	...
	const eventCards = (id, headline, posterUrl1, date, sale, location) => {
		return (
			<div
				key={id}
				className="w-full text-[#0e0c0a] bg-white border border-[#656565]  flex flex-col overflow-hidden shadow-sm-light shadow-[#25201b] rounded-md"
			>
				<div className="w-full h-2/5 border-b-[#e37027]">
					<img className="w-full h-full" src={posterUrl1} alt="" />
				</div>
				<div className="w-full h-3/5  flex flex-col px-4 py-3 gap-20">
					<div className="w-full">
						<p className="text-sm mb-3 text-gray-500">
							<Icon className="mr-2" icon={faMapLocation} size="sm" />
							{location}
						</p>
						<h1 className="line-clamp-1 font-semibold mb-1">{headline}</h1>
					...
					<div className="bottom-0 py-2 border-t w-full">
		...
		<section className="w-full mt-8">
			<main className="container">
				...
				<div className="w-full flex justify-end gap-5 p-4 max-sm:hidden">
					<button
						className={`hover:text-gray-600 ${index === 0 ? "text-gray-400" : "text-[#e37027]"}`}
						onClick={previous}
						disabled={index === 0}
					>
					...
					<button
						className={`hover:text-gray-600 ${index === slidesToShow ? "text-gray-400" : "text-[#e37027]"}`}
						onClick={next}
